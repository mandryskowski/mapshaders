...
class TileMapBase : public godot::Resource {
 ...
 public:
  ...
  /**
   * Turn a coordinate in geo space into a tile coordinate.
   * @param coord The geo space coordinate to convert (in Vector2
   * representation).
   * @return The tile coordinate.
   */
  ...
  virtual godot::TypedArray<godot::Vector2i> get_tiles_of_interest(
      GeoCoords coords, double elevation, godot::Vector3 front_vec) = 0;
...
class EquirectangularTileMap : public TileMapBase {
 ...
 public:
  EquirectangularTileMap(bool use_geo = false)
      : TileMapBase(use_geo), tile_size(1000.0, 1000.0) {}
