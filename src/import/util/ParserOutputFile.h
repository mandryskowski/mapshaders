...
class ParserOutputFile : public godot::RefCounted {
 ...
 public:
  ParserOutputFile()
      : parser_count(0), filename("") {}  // Required by godot; do not use
  /**
   * @brief Construct a new ParserOutputFile object
   * @param filename The filename to save to
   * @param parser_counts The number of parsers per slot
   */
  ParserOutputFile(const godot::String& filename,
                   const std::vector<unsigned int>& parsers_in_slot)
      : filename(filename), parser_count(0) {
    // Calculate offsets (prefix sum)
    for (int i = 0; i < parsers_in_slot.size(); i++) {
      const_cast<std::vector<unsigned int>&>(slot_offsets)
          .push_back(parser_count);
      const_cast<unsigned int&>(parser_count) += parsers_in_slot[i];
    }
  }
  godot::StreamPeerBuffer* get_parser(const godot::Vector2i& tile,
                                      unsigned int slot, unsigned int parser);
  void save();
  ...
  /**
   * @brief Get tile file access. It is your responsibility to close the file
   * access.
...
class ParserOutputFileHandle : public godot::RefCounted {
 ...
 public:
  ...
  ParserOutputFileHandle(godot::Ref<ParserOutputFile> file, unsigned int slot)
      : file(file), slot(slot) {}
  godot::StreamPeerBuffer* get_parser(const godot::Vector2i& tile,
                                      unsigned int parser) {
    return file->get_parser(tile, slot, parser);
  }
  unsigned int get_parser_count() const { return file->get_parser_count(slot); }
