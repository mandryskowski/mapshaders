...
StreamPeerBuffer* ParserOutputFile::get_parser(const Vector2i& tile,
                                               unsigned int slot,
                                               unsigned int parser) {
...
void ParserOutputFile::save() {
  ...
  // Tile space rect
  Vector2i min_tile(std::numeric_limits<int>::max(),
  ...
  WARN_PRINT("Tile space rect: " + String::num_int64(min_tile.x) + " " +
             ...
             String::num_int64(max_tile.x) + " " +
             String::num_int64(max_tile.y));

  Vector2i tile_space_size = max_tile - min_tile + Vector2i(1, 1);
  if (tile_space_size.x > 500 || tile_space_size.y > 500) {
    WARN_PRINT(
        "Tile space bounds too large: " + String::num_int64(tile_space_size.x) +
  ...
  for (int y = 0; y < tile_space_size.y; y++) {
    for (int x = 0; x < tile_space_size.x; x++) {
      ...
      for (int j = 0; j < tile_fas.size(); j++) {
        Ref<StreamPeerBuffer> fa =
            static_cast<Ref<StreamPeerBuffer>>(tile_fas[j]);
