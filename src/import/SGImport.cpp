...
void SGImport::import(bool) {
  ...
  for (int i = 0; i < this->parsers.size(); i++) {
    ...
    if (!parser->is_enabled()) continue;
    Ref<ParserOutputFileHandle> output_file_handle =
        memnew(ParserOutputFileHandle(output_file, i));

    if (Ref<OSMParser> osm_parser = Object::cast_to<OSMParser>(parser.ptr());
        osm_parser.is_valid()) {
      auto returned_geomap =
          osm_parser->import(output_file_handle, this->geomap, this->heightmap);

      if (!this->geomap.is_valid()) {
        this->set_geo_map(returned_geomap);
      }
    } else if (Ref<ElevationParser> elevation_parser =
                   Object::cast_to<ElevationParser>(parser.ptr());
               elevation_parser.is_valid()) {
      auto returned_grid =
          elevation_parser->import(output_file_handle, this->geomap);
      if (!this->geomap.is_valid()) {
        this->set_geo_map(returned_grid->get_geo_map());
      }
      this->heightmap =
          godot::Ref<OSMHeightmap>(memnew(ElevationHeightmap(returned_grid)));
    } else if (Ref<CoastlineParser> coastline_parser =
                   Object::cast_to<CoastlineParser>(parser.ptr());
               coastline_parser.is_valid()) {
...
void SGImport::import_elevation(bool) {
  ...
  for (int i = 0; i < this->parsers.size(); i++) {
    Ref<Parser> parser = this->parsers[i];
    if (!parser->is_enabled()) continue;
    Ref<ElevationParser> elevation_parser =
        Object::cast_to<ElevationParser>(*parser);
...
void SGImport::import_coastline(bool) {
  for (int i = 0; i < this->parsers.size(); i++) {
    Ref<Parser> parser = this->parsers[i];
    if (!parser->is_enabled()) continue;
    Ref<CoastlineParser> coastline_parser =
        Object::cast_to<CoastlineParser>(parser.ptr());
...
void SGImport::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("import_osm", "plsrefactor"),
                       &SGImport::import_osm);
  ClassDB::bind_method(D_METHOD("import_elevation", "plsrefactor"),
                       &SGImport::import_elevation);
  ...
  ClassDB::bind_method(D_METHOD("set_output_filename", "filename"),
                       &SGImport::set_output_filename);
  ClassDB::bind_method(D_METHOD("get_output_filename"),
                       &SGImport::get_output_filename);

  ClassDB::bind_method(D_METHOD("get_geo_map"), &SGImport::get_geo_map);
  ClassDB::bind_method(D_METHOD("set_geo_map", "geomap"),
                       &SGImport::set_geo_map);

  ClassDB::bind_method(D_METHOD("reset_geo_info"), &SGImport::reset_geo_info);

  ClassDB::bind_method(D_METHOD("set_parsers", "parsers"),
                       &SGImport::set_parsers);
  ClassDB::bind_method(D_METHOD("get_parsers"), &SGImport::get_parsers);

  ADD_PROPERTY(
      PropertyInfo(Variant::OBJECT, "geo_map", PROPERTY_HINT_RESOURCE_TYPE,
                   "EquirectangularGeoMap,SphereGeoMap"),
      "set_geo_map", "get_geo_map");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "Reset geo information"),
               "reset_geo_info", "get_true");

  ADD_PROPERTY(PropertyInfo(Variant::STRING, "output_filename",
                            PROPERTY_HINT_FILE, "*.sgdmap"),
               "set_output_filename", "get_output_filename");

  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "import"), "import", "get_true");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "import_osm"), "import_osm",
               "get_true");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "import_elevation"),
               "import_elevation", "get_true");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "import_coastline"),
               "import_coastline", "get_true");

  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "load_tiles"), "load_tiles",
               "get_true");

  ADD_PROPERTY(
      PropertyInfo(Variant::ARRAY, "parsers", PROPERTY_HINT_TYPE_STRING,
                   vformat("%s/%s:%s", Variant::OBJECT,
                           PROPERTY_HINT_RESOURCE_TYPE, "Parser"),
                   PROPERTY_HINT_ARRAY_TYPE),
      "set_parsers", "get_parsers");
  // ADD_SIGNAL()
  // ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "latitude", PROPERTY_HINT_RANGE,
  // "-90,90,0.01"), "set_origin_latitude", "get_origin_latitude");
}

Ref<ParserOutputFile> SGImport::get_parser_output_file() {
  std::vector<unsigned int> parsers_in_slot;
  for (int i = 0; i < this->parsers.size(); i++) {
    parsers_in_slot.push_back(
        Object::cast_to<Parser>(this->parsers[i])->get_shader_nodes().size());
  }
