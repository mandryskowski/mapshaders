...
However, 1 degree of longitude is a different distance depending on the
latitude.
*/

/* SPACES:
    - Earth space: Our planet represented by geographical coordinates.
    - World space: 3D Euclidean space of our Godot scene represented by
   Cartesian coordinates.
    - Grid space: Earth data is partitioned into a grid to allow loading just a
   part of it. Represented by 2D Cartesian coordinates. The origin corresponds
   to the origin of World space. The grid must have odd dimensions, e.g. 3x3,
...
struct GeoCoords {
  Longitude lon;
  Latitude lat;
  GeoCoords(Longitude lon = Longitude::radians(0),
            Latitude lat = Latitude::radians(0))
      : lon(lon), lat(lat) {}
   ...
   *        Only use if you really need a Vector2 representation (e.g. for Godot
   * array). Please avoid storing geographical coordinates in Vector2 as it is
   * less readable.
   */
  godot::Vector2 to_vector2_representation() const {
    return godot::Vector2(static_cast<real_t>(lon.get_radians()),
                          static_cast<real_t>(lat.get_radians()));
  }

  /**
   * @brief Converts a 2D vector representation of the GeoCoords back to
   * GeoCoords.
   */
...
class GeoMap : public godot::Resource {
 ...
 public:
  ...
  /* Optional scale factor for your convenience if you're not using 1 unit = 1
   * m. */
  void set_scale_factor(double factor) { this->scale_factor = factor; }
...
class OriginBasedGeoMap : public GeoMap {
 ...
 protected:
  ...
  /* Geo coordinates of the place on Earth corresponding to (0, 0) in world
   * space. */
...
#define LATITUDE_DEGREE_IN_METRES 111139.0
 ...
 protected:
  MAPSHADERS_DLL_SYMBOL virtual godot::Vector3 geo_to_world_impl(
      GeoCoords) override;
  static void _bind_methods();
};
...
class SphereGeoMap : public GeoMap {
 ...
 public:
  /* Converts Earth space coordinates into World space. */
  MAPSHADERS_DLL_SYMBOL virtual godot::Vector3 geo_to_world_up(
      GeoCoords) override;
