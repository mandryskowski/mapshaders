...
void SkeletonSubtree::set_source(const Vector2& value) { source = value; }
double SkeletonSubtree::get_height() const { return height; }
void SkeletonSubtree::set_height(double value) { height = value; }
PackedVector2Array SkeletonSubtree::get_sinks() const { return sinks; }
...
void SkeletonSubtree::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("set_source", "value"),
                       &SkeletonSubtree::set_source);
  ClassDB::bind_method(D_METHOD("get_height"), &SkeletonSubtree::get_height);
  ClassDB::bind_method(D_METHOD("set_height", "value"),
                       &SkeletonSubtree::set_height);
  ClassDB::bind_method(D_METHOD("get_sinks"), &SkeletonSubtree::get_sinks);
  ClassDB::bind_method(D_METHOD("set_sinks", "value"),
                       &SkeletonSubtree::set_sinks);

  ADD_PROPERTY(PropertyInfo(Variant::VECTOR2, "source"), "set_source",
               "get_source");
  ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "height"), "set_height",
               "get_height");
  ADD_PROPERTY(PropertyInfo(Variant::PACKED_VECTOR2_ARRAY, "sinks"),
               "set_sinks", "get_sinks");
}
...
bool polygon_invalid_edges_check(const std::vector<p2t::Point*>& polygon) {
  ...
  for (int i = 0; i < polygon.size() - 1; i++) {
    int j = i + 1;
    if (std::abs(polygon[i]->x - polygon[j]->x) < epsilon &&
        std::abs(polygon[i]->y - polygon[j]->y) < epsilon) {
...
PackedVector2Array PolyUtil::triangulate_with_holes(PackedVector2Array outer,
                                                    Array holes) {
  // WARN_PRINT("Running triangulate w holes w outer size: " +
  // String::num_int64(outer.size()) + " hole count: " +
  // String::num_int64(holes.size()));
  //  Create polyline and CDT
  std::vector<p2t::Point*> polyline;
  polyline.reserve(outer.size());
  for (const auto& v : outer) polyline.push_back(new p2t::Point(v.x, v.y));

  if (!polygon_invalid_edges_check(polyline)) return PackedVector2Array();
  ...
  // Free
  for (p2t::Point* v : polyline);  // delete v;
...
MAPSHADERS_DLL_SYMBOL std::vector<GeoCoords> PolyUtil::triangulate_with_holes(
  ...
  if (!polygon_invalid_edges_check(polyline)) return std::vector<GeoCoords>();
  ...
  for (const auto& triangle : triangles) {
    for (int i = 0; i < 3; i++) {
      p2t::Point* p = triangle->GetPoint(i);
      triangulated.push_back(
          GeoCoords(Longitude::radians(p->x), Latitude::radians(p->y)));
    }
  }
...
PolyUtil::triangulate_with_holes(
...
Array PolyUtil::straight_skeleton(PackedVector2Array outer, Array holes) {
  // std::cout << "Running straight skeleton with outer size: " << outer.size()
  // << " hole count: " << holes.size() << std::endl;
  ...
  std::vector<std::shared_ptr<polyskel::Subtree>> result =
      polyskel::skeletonize(polyskel_outer, polyskel_holes);
...
void PolyUtil::_bind_methods() {
  ClassDB::bind_method(
      D_METHOD("triangulate_with_holes", "outer", "holes"),
      (PackedVector2Array(PolyUtil::*)(
          PackedVector2Array, Array))&PolyUtil::triangulate_with_holes);
  ClassDB::bind_method(D_METHOD("straight_skeleton", "outer", "holes"),
                       &PolyUtil::straight_skeleton);
}
