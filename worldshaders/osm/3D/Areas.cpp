...
void Areas::import_node(godot::Dictionary d,
                        godot::Ref<godot::StreamPeerBuffer> fa) {
  node_pos[d["id"]] = GeoCoords(Longitude::radians(d["pos_geo_lon"]),
                                Latitude::radians(d["pos_geo_lat"]));
}

void Areas::import_way(godot::Dictionary d,
                       godot::Ref<godot::StreamPeerBuffer> fa) {
  if (!tile_info.has(fa)) {
    Dictionary fa_d;
  ...
  static_cast<Dictionary>(
      static_cast<Dictionary>(tile_info[fa])["ways"])[d["id"]] = d;
  ways[d["id"]] = d;
}

void Areas::import_relation(godot::Dictionary d,
                            godot::Ref<godot::StreamPeerBuffer> fa) {
  ...
  outd["rooftris"] = RenderUtil3D::geo_polygon_to_world(
  ...
  if (d.has("height")) outd["max_height"] = d["height"];
  if (d.has("min_height")) outd["min_height"] = d["min_height"];

  static_cast<Array>(static_cast<Dictionary>(tile_info[fa])["rels"])
      .append(outd);
}
...
void Areas::load_tile(godot::Ref<godot::FileAccess> fa) {
  ...
  for (int i = 0; i < rels.size(); i++) {
    ...
    if (!rel.has("rooftris")) continue;

    Node* hole_area = RenderUtil3D::achild(this, memnew(Node),
                                           "HoleArea" + (String)rel["name"]);
    PackedVector3Array rooftris =
        static_cast<PackedVector3Array>(rel["rooftris"]);

    double min_height = rel.get("min_height", 0.0);
    double max_height = rel.get("max_height", 0.0);

    RenderUtil3D::area_poly(
        hole_area, "Roof" + (String)rel["name"],
